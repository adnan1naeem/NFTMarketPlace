...
export const checkAlreadyConnectedMetaMask =
  (currentConnection) => async (dispatch) => {
    if (window.web3) {
      ...
      if (window.ethereum.isMetaMask) {
        await window.web3.eth.getAccounts((error, accounts) => {
          if (accounts?.length) {
            ...
            dispatch(
              connectMetaMaskAction({ connection: true, address: accounts })
            )
          } else {
...
const updateAddress = async (dispatch) => {
  ...
  if (accounts) {
    ...
    web3.eth
      ...
      .then((res) => dispatch(setUserBalance({ flag: true, balance: res })))
    refreshPage()
  } else {
    dispatch(connectMetaMaskAction({ connection: false, address: [] }))
    dispatch(setUserBalance({ flag: false, balance: 0 }))
